...
gulp.task('packagejson', function() {
  return gulp.src(['package.json'])
      .pipe(jsonTransform(
          (data, file) => {
            // scripts: delete this section, because it will be mostly
            // non-functional
            if (!!data.scripts) {
              delete data.scripts;
            }
...
gulp.task('npmignore', function() {
  ...
  return gulp.src('.npmignore').pipe(gulp.dest(outDir));
...
gulp.task('build', ['packagejson', 'npmignore', 'tsc', 'tslint']);
...
gulp.task('watch', ['build'], function() {
  watch(watching, batch(function(events, done) { gulp.start('tsc', done); }));
});
