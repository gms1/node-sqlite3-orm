           ...
           new Promise<void>(async(resolve, reject) => {
             try {
               ...
               expect(user1.userId)
                   .toBe(
                       user2.userId,
                       'userId does not match after first update');
               ...
               expect(user1.userId)
                   .toBe(
                       user2.userId, 'userId does not match after select all');
               expect(user1.userLoginName)
                   .toBe(
                       user2.userLoginName,
               ...
               reject(new Error('test BaseDAOs basic functionality failed: ' + e.message));
             }
             resolve();
           })
               .then((res) => done())
       ...
       let p =
           new Promise<void>(async(resolve, reject) => {
             try {
               ...
               user.userId = 1;
               let contactsUser1$1 = await contactDao.selectAllOf(Contact.userConstraint, User, user, ' AND contact_email=:contact_email', {':contact_email': 'user1@test2.net'});
               expect(contactsUser1$1.length).toBe(1);

             } catch (e) {
               reject(new Error('test BaseDAO foreign keys failed: ' + e.message));
             }
             resolve();
           })
               .then((res) => done())
