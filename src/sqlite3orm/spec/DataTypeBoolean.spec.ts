...
describe('test boolean type', () => {
  ...
  it('expect writing boolean properties to the database to succeed',
     async(done) => {
       try {
  ...
  it('expect reading boolean properties from database to succeed',
     async(done) => {
       try {
         let sqlstmt =
             await sqldb.prepare(`INSERT INTO ${DATATYPE_BOOLEAN_TABLE}
              (id, my_bool_text, my_bool_int, my_bool_real)
            values
              (:id, :my_bool_text, :my_bool_int, :my_bool_real)`);
         ...
         expect(typeof model.myBool2Text)
             .toBe(
                 'boolean',
                 `record ${model.id}: myBool2Text should be of type boolean`);
         expect(typeof model.myBool2Int)
             .toBe(
                 'boolean',
                 `record ${model.id}: myBool2Int should be of type boolean`);
         expect(typeof model.myBool2Real)
             .toBe(
                 'boolean',
                 `record ${model.id}: myBool2Real should be of type boolean`);
         expect(model.myBool2Text)
             .toBeTruthy(`record ${model.id}: myBool2Text should be true`);
         expect(model.myBool2Int)
             .toBeTruthy(`record ${model.id}: myBool2Int should be true`);
         expect(model.myBool2Real)
             .toBeTruthy(`record ${model.id}: myBool2Real should be true`);
         ...
         expect(typeof model.myBool2Real)
             .toBe(
                 ...
                 `record ${model.id}: myBool2Real should be of type boolean`);
         expect(model.myBool2Text)
             .toBeFalsy(`record ${model.id}: myBool2Text should be false`);
         expect(model.myBool2Int)
             .toBeFalsy(`record ${model.id}: myBool2Int should be false`);
         expect(model.myBool2Real)
             .toBeFalsy(`record ${model.id}: myBool2Real should be false`);
